    ...
    <div className="mx-auto flex w-full max-w-5xl flex-col items-center gap-5">
      ...
      <div className="flex w-full flex-col gap-5 md:flex-row md:items-stretch">
        <div className="grid flex-1 grid-cols-3 gap-3">
          {lobby.players.map((p) => (
            <div key={p.user.id} className="flex flex-col items-center justify-center gap-1.5 rounded-lg border border-dl-border bg-black/40 p-3">
          ...
          {Array.from({ length: Math.max(0, 6 - lobby.players.length) }).map((_, i) => (
            <div
              key={`empty-${i}`}
              className="flex flex-col items-center justify-center gap-1.5 rounded-lg border border-dashed border-dl-border/60 p-3 text-dl-text/30"
            >
